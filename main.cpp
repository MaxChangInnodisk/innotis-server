...
int main(int argc, char** argv) {
    ...
    try {
        ...
        else if (network_string.compare("yolov4will") == 0) {
            network = NETWORKS::YOLOV4WILL;
        }
        else {
    ...
    else if(network == NETWORKS::YOLOV4WILL) {
        std::cout << "[Info] Creating model yolov4will" << std::endl;
        engine = yolov4will::createEngine(BATCH_SIZE, builder, config, DataType::kFLOAT, "yolov4will.wts");
    }
    ...
    std::string engine_name = network == NETWORKS::YOLOV4TINY3L ? "yolov4tiny3l.engine" : ( network == NETWORKS::YOLOV4TINY ? "yolov4tiny.engine" : ( network == NETWORKS::YOLOV4 ? "yolov4.engine" : "yolov4custom.engine" ) );
